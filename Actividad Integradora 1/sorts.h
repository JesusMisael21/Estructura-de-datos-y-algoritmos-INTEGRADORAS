...
template <class T>
void Sorts<T>::separar_atributos(std::vector<T>&velocidad,std::vector<T>&nombre,std::vector<T>&datos){
	int l=0;
	for(int k=0;k<datos.size();k++){
		if(l%2!=0){velocidad.push_back(datos[k]);}
...
template <class T>
std::vector<T> Sorts<T>::unir_atributos(std::vector<T>&velocidad,std::vector<T>&nombre){
	std::vector<T> datos_ordenados;
	for(int m=0;m<velocidad.size();m++){
		datos_ordenados.push_back(nombre[m]);
		datos_ordenados.push_back(velocidad[m]);
	}
...
std::vector<T> Sorts<T>::selectionSort(const std::vector<T> &source) {
	...
	std::vector<T> nombre;
	std::vector<T> velocidad;
	separar_atributos(velocidad,nombre,datos);
	int pos;

	for (int i = velocidad.size() - 1; i > 0; i--) {
		pos = 0;
		for (int j = 1; j <= i; j++) {
			if (velocidad[j] > velocidad[pos]) {
				pos = j;
			}
		}

		if (pos != i) {
			swap(velocidad, i, pos, nombre);
		}
	}
	return unir_atributos(velocidad,nombre);
}
...
std::vector<T> Sorts<T>::Busqueda(const std::vector<T>&source,int min,int max){
	...
	for(int i=posI;i<=posF;i++){
		datosBuscados.push_back(nombre[i]+" "+velocidad[i]);
	}
