...
// definición de la función update (actualiza) 
template <class T>
bool Node<T>::update(T val,T act) {
	if (val == value) {
		value=act;
		return true;
	} else if (val < value) {
		return (left != 0 && left->update(val,act));
	} else if (val > value) {
		return (right != 0 && right->update(val,act));
	}
	return false;
}
...
// definición de la función update (actualiza)
template <class T>
bool BST<T>::update(T val,T act) const {
	if (root != 0) {
		return root->update(val,act);
	}
	else {
		return false;
	}
